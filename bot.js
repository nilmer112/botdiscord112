...
var ownerID		= "325349909412970496";
var ownerID2	= "325349909412970496";
...
bot.login(NjIyNjkzNzU5NDYwODM1MzM5.XX3m3Q.KmRbcK5ZeaVXAUSX05v50Tnyx6g);
bot.on('ready',async () => {
...
bot.on('message', async message => {
	...
	switch(command){
		...
		case 'info': {
			...
			.setColor(color)
			.setAuthor('ShieldBot', bot_image, 'https://crysgfx.com/')
			.setDescription('CrysADV Personal bot')
			.setThumbnail(bot_image)
			.addField('Bot OWNER/Editor', 'CrysADV')
			.addField('Special Thanks', 's0nyx', true)
			.setTimestamp()
		...
		case 'invite': {
		 let user = message.mentions.users.first() || message.author;
			...
			.setAuthor('Click here for invite!', bot_image, 'https://discordapp.com/oauth2/authorize?client_id=607513685753069578&scope=bot&permissions=8')
			.setColor(color)
			.setTitle(`Invite ShieldBOT to your discord server!`)
		...
		case 'dmall': {
			...
			if(!text)return message.channel.send('```Syntax: !dm [text]```');
		...
		case 'addalt': {
			if(message.author.id == "325349909412970496" || message.author.id == "325349909412970496"){
				var type = args[1];
		...
		case 'addrole': {
		  ...
		  .addField("Example", "$addrole @ShieldBot Member")
		...
		case 'removerole': {
		  ...
		  .addField("Example", "$removerole @ShieldBot Member")
